...
function ticketStrip(p) {
  const g = p.g; const sideTxt = (g.side === 'long' ? 'Long ' : 'Short ') + p.sym;
  return `<div class="ticket"><div class="ticket-strip">
    <div class="tf"><b>Side</b><i>${esc(sideTxt)}</i></div>
    <div class="tf"><b>Entry</b><i>${fmtPrice(g.entry, p.sym)}</i></div>
    <div class="tf"><b>Stop</b><i>${fmtPrice(g.stop, p.sym)}</i></div>
    <div class="tf hide-m"><b>Target</b><i>${fmtPrice(g.target, p.sym)}</i></div>
    <div class="tf hide-m"><b>Exit</b><i>${fmtPrice(g.exit, p.sym)}</i></div>
    <div class="stamp ${g.r > 0 ? 'win' : 'loss'}">${rTxt(g.r)}</div>
  </div></div>`;
}
function mediaHTML(p, { reel = false } = {}) {
  const tag = `<span class="tagl">${esc(p.sym)} ${esc(p.tf)}</span>`;
  if (p.type === 'image') return `<img src="${p.url}" alt="Chart posted by ${esc(p.user)}">${tag}`;
  if (p.type === 'video') return `<video src="${p.url}" playsinline muted loop ${reel ? 'autoplay' : 'controls'} preload="metadata"></video>${tag}`;
  if (p.type === 'replay') return `<canvas data-replay="${p.id}" role="img" aria-label="Trade replay of ${esc(p.sym)}, ${rTxt(p.g.r)}"></canvas>${tag}<button class="play" data-act="toggle-replay" data-id="${p.id}">${I.pause}<span>Replay</span></button><div class="bar"><i></i></div>`;
  return `<img src="${chartImage(p.g, p.id)}" alt="${esc(p.sym)} ${p.g.side} trade, ${rTxt(p.g.r)}" loading="lazy">${tag}`;
}
function postHTML(p) {
  ...
  return `<article class="post" data-post="${p.id}">
    ...
    <div class="acts">
      <button class="act" data-act="like" data-id="${p.id}" aria-pressed="${liked}" aria-label="Like">${I.heart}<span>${compact(p.likes + (liked ? 1 : 0))}</span></button>
    ...
    ${ticketStrip(p)}
    <p class="caption"><a href="#/u/${esc(p.user)}">${esc(u.handle)}</a>${esc(p.caption)}</p>
...
function hydrateMedia(root = document) {
  $$('canvas[data-replay]', root).forEach((cv) => {
    if (cv._rp) return; const p = P(cv.dataset.replay); if (!p) return;
    const media = cv.closest('.media, .phone-screen'); const bar = media && $('.bar i, .phone-bar i', media);
    cv._rp = mountReplay(cv, p.g, { onTick: (t) => bar && (bar.style.width = (t * 100).toFixed(1) + '%') });
    const btn = media && $('.play', media);
    if (btn) { const rp = cv._rp; const upd = () => { btn.innerHTML = (rp.playing ? I.pause : I.play) + '<span>Replay</span>'; btn.setAttribute('aria-label', rp.playing ? 'Pause replay' : 'Play replay'); };
      const op = rp.play, ps = rp.pause; rp.play = () => { op(); upd(); }; rp.pause = () => { ps(); upd(); }; rp._upd = upd; upd(); }
  });
}

/* ============ SHELL ============ */
const NAV = [['floor', 'The Floor', I.floor], ['replays', 'Replays', I.replay], ['discover', 'Discover strategies', I.discover], ['builder', 'Strategy builder', I.builder], ['u/' + ME, 'Profile', I.profile]];
function shell(active, inner) {
  const me = U(ME);
  const links = NAV.map(([k, l, ic]) => `<a href="#/${k}" ${active === k.split('/')[0] ? 'aria-current="page"' : ''}>${ic}<span>${l}</span></a>`).join('');
  const tabs = NAV.map(([k, l, ic]) => `<a href="#/${k}" aria-label="${l}" ${active === k.split('/')[0] ? 'aria-current="page"' : ''}>${ic}</a>`).join('');
  return `<div class="shell">
    <aside class="rail"><a class="mark" href="#/">The<br>Trading<br>Floor</a>
      ...
      <a class="me" href="#/u/${ME}">${badge(ME, 36)}<div><strong>${esc(me.handle)}</strong><small>Badge ${esc(me.badge)}</small></div></a>
    </aside>
    <header class="topbar"><a class="mark" href="#/">The Trading Floor</a><button class="btn btn-floor btn-sm" data-act="compose">${I.plus}Post</button></header>
...
function feedView() {
  ...
  if (f === 'charts') list = list.filter((p) => p.type === 'chart' || p.type === 'image');
  if (f === 'replays') list = list.filter((p) => p.type === 'replay' || p.type === 'video');
  if (f === 'wins') list = list.filter((p) => p.g.r > 0);
  if (f === 'losses') list = list.filter((p) => p.g.r <= 0);
  const chips = [['all', 'For you'], ['following', 'Following'], ['charts', 'Charts'], ['replays', 'Replays'], ['wins', 'Wins'], ['losses', 'Losses']]
    .map(([k, l]) => `<button class="chip" data-act="feed-filter" data-f="${k}" aria-pressed="${f === k}">${l}</button>`).join('');
  const tape = [...state.posts].sort((a, b) => b.t - a.t).slice(0, 10);
  const tapeLi = tape.map((p) => `<li>${badge(p.user, 28)}<span><b>${esc(p.user)}</b> ${p.g.side} ${esc(p.sym)}</span><i class="${p.g.r > 0 ? 'w' : ''}">${rTxt(p.g.r)}</i></li>`).join('');
  const rising = [...state.strategies].sort((a, b) => b.followers / (b.created + 3) - a.followers / (a.created + 3)).slice(0, 4)
  ...
  return shell('floor', `<div class="cols"><section aria-label="Feed">
      <div class="feed-tabs" role="group" aria-label="Filter feed">${chips}</div>
      ${list.length ? list.map(postHTML).join('') : `<div class="empty">Nothing here yet. <br><button class="btn btn-floor" data-act="compose">Post a trade</button></div>`}
    </section>
    <aside class="side" aria-label="Live activity">
      <div><h3>The tape</h3><div class="side-tape"><ul>${tapeLi}${tapeLi}</ul></div></div>
      <div><h3>Strategies on the rise</h3>${rising}<a class="btn btn-line btn-sm" style="margin-top:14px" href="#/discover">Discover strategies</a></div>
    </aside></div>`);
}

/* ============ REPLAYS (vertical) ============ */
function replaysView() {
  const list = state.posts.filter((p) => p.type === 'replay' || p.type === 'video').sort((a, b) => b.t - a.t);
  const reels = list.map((p) => { const s = p.strategy && S(p.strategy); const liked = state.liked.has(p.id);
    return `<section class="reel" data-post="${p.id}"><div class="phone"><div class="phone-screen media">${mediaHTML(p, { reel: true })}
      <div class="phone-ov"><div class="who">${badge(p.user, 28)}<a href="#/u/${esc(p.user)}">${esc(p.user)}</a></div><p>${esc(p.caption)}</p>${ticketStrip(p)}</div></div></div>
      <div class="reel-acts">
        <button class="act" data-act="like" data-id="${p.id}" aria-pressed="${liked}" aria-label="Like">${I.heart}<span>${compact(p.likes + (liked ? 1 : 0))}</span></button>
  ...
  return shell('replays', `<div class="reels">${reels || '<div class="empty" style="margin:40px">No replays yet.</div>'}</div>`);
}
...
function scardHTML(s) {
  ...
  return `<a class="scard" href="#/s/${s.id}"><div class="ticket">
    ...
    <div class="body"><p>${esc(s.tagline)}</p>
      <div class="by">${badge(s.user, 26)}<span>${esc(u.handle)}</span></div>
...
function discoverView() {
  ...
  return shell('discover', `<div class="page">
    <div class="page-h"><div><h1>Discover strategies</h1><p>Step-by-step playbooks published by traders on the floor. Follow one to see its trades in your feed, or fork it into your own.</p></div>
...
function renderDiscGrid() {
  ...
  list.sort((a, b) => (disc.sort === 'new' ? a.created - b.created : disc.sort === 'forks' ? b.forks - a.forks : b.followers - a.followers));
  $('#sgrid').innerHTML = list.length ? list.map(scardHTML).join('') : `<div class="empty" style="grid-column:1/-1">No strategies match. Try another market or style, or <a href="#/builder" style="text-decoration:underline">publish the first one</a>.</div>`;
}

/* ============ STRATEGY SITE ============ */
function stepShot(st, key) {
  if (st.img) return `<div class="shot"><img src="${st.img}" alt="Example for ${esc(st.title)}"></div>`;
  if (st.chart) { const g = genTrade({ seed: st.chart.seed, sym: 'MES', side: st.chart.side, outcome: st.chart.outcome }); return `<div class="shot"><img src="${chartImage(g, 'st' + key + st.chart.seed, 800, 600)}" alt="Example chart for ${esc(st.title)}"></div>`; }
  return '';
}
function strategyHTML(s, { preview = false } = {}) {
  const u = U(s.user); const following = state.followedStrategies.has(s.id); const mine = s.user === ME;
  const steps = (s.steps.length ? s.steps : [{ kind: 'Setup', title: 'Your first step', body: 'Add steps on the left.', checks: [] }]).map((st, i) => {
    const shot = stepShot(st, (s.id || 'draft') + i);
    return `<div class="sp-step"><div class="idx">${String(i + 1).padStart(2, '0')}</div><div class="ticket ${shot ? '' : 'nochart'}"><div>
      ...
      ${st.checks && st.checks.length ? `<ul class="checks">${st.checks.map((c, j) => `<li><label><input type="checkbox" ${preview ? 'tabindex="-1"' : ''} aria-label="${esc(c)}"><span>${esc(c)}</span></label></li>`).join('')}</ul>` : ''}
    </div>${shot}</div></div>`; }).join('');
  const used = preview ? [] : state.posts.filter((p) => p.strategy === s.id);
  const st = s.stats || {};
  return `<div class="sp t-${s.theme}">
    <div class="sp-url"><span class="dots"><i></i><i></i><i></i></span>thetradingfloor.app/<b>@${esc(s.user)}/${esc(s.id || slug(s.title))}</b></div>
    <header class="sp-cover"><div class="meta"><span>${esc(s.market)}</span><span>${esc(s.session)} session</span><span>${esc(s.timeframe)}</span><span>${esc(s.style)}</span></div>
      ...
      <div class="by">${badge(s.user, 44)}<div class="who"><a href="#/u/${esc(s.user)}"><strong>${esc(u.handle)}</strong></a><small>${compact(s.followers || 0)} following, ${s.forks || 0} forks</small></div>
...
function strategyView(id) {
  const s = S(id); if (!s) return shell('discover', `<div class="page"><div class="empty">That strategy does not exist or was removed. <br><a class="btn btn-floor" href="#/discover">Discover strategies</a></div></div>`);
...
function thumbHTML(p) {
  const src = p.type === 'image' ? p.url : p.type === 'video' ? null : chartImage(p.g, p.id);
  return `<button class="thumb" data-act="open-post" data-id="${p.id}" aria-label="Open ${esc(p.sym)} trade, ${rTxt(p.g.r)}">${src ? `<img src="${src}" alt="" loading="lazy">` : `<video src="${p.url}" muted preload="metadata"></video>`}
    <span class="r ${p.g.r > 0 ? 'w' : ''}">${rTxt(p.g.r)}</span>${p.type === 'replay' || p.type === 'video' ? '<span class="rp">Replay</span>' : ''}</button>`;
}
function profileView(h) {
  const u = U(h); if (!u) return shell('u', `<div class="page"><div class="empty">No trader with that badge.</div></div>`);
  const posts = state.posts.filter((p) => p.user === h).sort((a, b) => b.t - a.t);
  const strats = state.strategies.filter((s) => s.user === h);
  const wins = posts.filter((p) => p.g.r > 0).length; const me = h === ME; const fol = state.following.has(h);
  const net = posts.reduce((a, p) => a + p.g.r, 0);
  let body = '';
  if (profTab === 'posts') body = posts.length ? `<div class="thumbs">${posts.map(thumbHTML).join('')}</div>` : `<div class="empty">No trades posted yet.${me ? '<br><button class="btn btn-floor" data-act="compose">Post a trade</button>' : ''}</div>`;
  if (profTab === 'replays') { const r = posts.filter((p) => p.type === 'replay' || p.type === 'video'); body = r.length ? `<div class="thumbs">${r.map(thumbHTML).join('')}</div>` : '<div class="empty">No replays yet.</div>'; }
  ...
  return shell(me ? 'u' : 'discover', `<div class="page"><header class="prof">${badge(h, 150)}<div>
      <h1>${esc(u.name)}</h1><div class="h">@${esc(u.handle)}, badge ${esc(u.badge)}</div><p>${esc(u.bio)}</p>
      <div class="stats"><span><b>${posts.length}</b>trades</span><span><b>${wins}/${posts.length}</b>winners</span><span><b>${rTxt(Math.round(net * 10) / 10)}</b>net</span><span><b>${compact(u.followers + (fol ? 1 : 0))}</b>followers</span><span><b>${strats.length}</b>strategies</span></div>
      <div class="row">${me ? `<button class="btn btn-floor" data-act="compose">${I.plus}Post a trade</button><a class="btn btn-line" href="#/builder">New strategy</a>` : `<button class="btn ${fol ? 'btn-line' : 'btn-floor'}" data-act="follow" data-h="${esc(h)}">${fol ? 'Following' : 'Follow'}</button>`}
      ${u.markets.map((m) => `<span class="chip" style="display:inline-grid;place-items:center">${esc(m)}</span>`).join('')}</div>
    </div></header>
    <div class="tabs" role="tablist">${[['posts', 'Trades'], ['replays', 'Replays'], ['strategies', 'Strategies']].map(([k, l]) => `<button role="tab" aria-selected="${profTab === k}" data-act="prof-tab" data-t="${k}">${l}</button>`).join('')}</div>
    ${body}</div>`);
}
...
function openPostModal(id) {
  ...
  openModal(`<div class="sheet" role="dialog" aria-modal="true" aria-label="Trade by ${esc(p.user)}" style="width:min(620px,100%)"><header><h2>${esc(p.sym)} ${p.g.side}</h2><button class="ib" data-act="close" aria-label="Close">${I.x}</button></header><div style="padding:20px">${postHTML(p)}</div></div>`);
}
...
function closeModal() { const m = $('#modal'); if (m) { $$('canvas[data-replay]', m).forEach((c) => c._rp && c._rp.destroy()); m.remove(); document.body.style.overflow = ''; } }
addEventListener('keydown', (e) => { if (e.key === 'Escape') closeModal(); });

/* ============ COMPOSER ============ */
let cmp;
function openComposer() {
  cmp = { side: 'long', sym: 'MGC', tf: '5m', session: 'London', entry: '', stop: '', target: '', exit: '', caption: '', strategy: '', file: null, ftype: null };
  ...
  openModal(`<div class="sheet" role="dialog" aria-modal="true" aria-labelledby="cmpH"><header><h2 id="cmpH">Post a trade</h2><button class="ib" data-act="close" aria-label="Close">${I.x}</button></header>
    <div class="compose"><div class="left">
      <div class="drop" id="drop"><div id="dropTxt"><strong>Add a screenshot or screen recording</strong>Drag it here, or leave it empty and we draw the chart from your ticket.<br><label class="btn btn-line btn-sm" style="margin-top:14px">Choose file<input type="file" accept="image/*,video/*" id="cmpFile" class="sr"></label></div></div>
    ...
    <div class="right">
      ...
      <div class="g2"><label class="field"><span>Entry</span><input class="input num" inputmode="decimal" data-k="entry" placeholder="3642.5"></label><label class="field"><span>Stop</span><input class="input num" inputmode="decimal" data-k="stop" placeholder="3638.0"></label>
        <label class="field"><span>Target</span><input class="input num" inputmode="decimal" data-k="target" placeholder="3654.0"></label><label class="field"><span>Exit</span><input class="input num" inputmode="decimal" data-k="exit" placeholder="3653.8"></label></div>
      <div class="rcalc" id="rcalc">Enter entry, stop and exit to see the result in R.</div>
      <label class="field"><span>Strategy used</span><select class="input" data-k="strategy"><option value="">None</option>${mine.map((s) => `<option value="${s.id}">${esc(s.title)}</option>`).join('')}</select></label>
...
function cmpFile(f) {
  if (!f) return; if (!/^(image|video)\//.test(f.type)) { $('#cmpErr').textContent = 'That file is not an image or video. Use PNG, JPG, GIF, MP4 or MOV.'; return; }
  if (f.size > 200 * 1024 * 1024) { $('#cmpErr').textContent = 'That file is over 200 MB. Trim the recording and try again.'; return; }
  cmp.file = URL.createObjectURL(f); cmp.ftype = f.type.startsWith('video') ? 'video' : 'image'; $('#cmpErr').textContent = ''; cmpUpdate();
}
...
function cmpUpdate() {
  ...
  const ok = isFinite(e) && isFinite(s) && isFinite(x) && e !== s;
  if (ok) { const R = Math.abs(e - s); const r = (cmp.side === 'long' ? x - e : e - x) / R; rc.innerHTML = `Risk <b>${R.toFixed(SYMBOLS[cmp.sym].dp)}</b> points, result <b>${rTxt(Math.round(r * 10) / 10)}</b>${isFinite(t) ? `, planned <b>${(Math.abs(t - e) / R).toFixed(1)}R</b>` : ''}.`; }
  else rc.textContent = 'Enter entry, stop and exit to see the result in R.';
  $('#cmpClear').hidden = !cmp.file;
  let pv = drop.querySelector('img, video');
  ...
  else if (ok) { const g = tradeFromTicket(cmpTicket(), 777); const url = (() => { const cv = document.createElement('canvas'); cv.width = 640; cv.height = 800; drawChart(cv.getContext('2d'), 640, 800, g, { litEntry: true }); return cv.toDataURL(); })(); pv && pv.remove(); drop.insertAdjacentHTML('beforeend', `<img src="${url}" alt="Chart drawn from your ticket">`); $('#dropTxt').style.visibility = 'hidden'; }
  else { pv && pv.remove(); $('#dropTxt').style.visibility = ''; }
}
function cmpTicket() { const { e, s, t, x } = cmpNums(); const R = Math.abs(e - s); return { sym: cmp.sym, side: cmp.side, entry: e, stop: s, exit: x, target: isFinite(t) ? t : cmp.side === 'long' ? e + 2 * R : e - 2 * R }; }
function cmpPost() {
  const { e, s, x } = cmpNums(); const err = $('#cmpErr');
  if (![e, s, x].every(isFinite)) return (err.textContent = 'Entry, stop and exit need to be numbers.');
  ...
  const id = 'u' + Date.now(); const tk = cmpTicket();
  const p = { id, user: ME, type: cmp.file ? cmp.ftype : 'chart', url: cmp.file, seed: Math.floor(Math.random() * 1e6), sym: cmp.sym, tf: cmp.tf, session: cmp.session, side: cmp.side, caption: cmp.caption.trim() || `${cmp.side === 'long' ? 'Long' : 'Short'} ${cmp.sym}.`, strategy: cmp.strategy || null, likes: 0, comments: [], t: Date.now() };
  p.g = tradeFromTicket(tk, p.seed); state.posts.unshift(p);
  closeModal(); feedFilter = 'all'; if (location.hash === '#/floor') render(); else location.hash = '#/floor'; toast('Posted to the floor');
}

/* ============ BUILDER ============ */
const KINDS = ['Setup', 'Trigger', 'Entry', 'Stop', 'Target', 'Management', 'Rule'];
function newDraft() { return { id: null, user: ME, title: '', tagline: '', market: 'MGC', session: 'London', timeframe: '5m', style: 'ICT', theme: 'floor', followers: 0, forks: 0, stats: { winRate: '', avgR: '', sample: '' }, steps: [{ kind: 'Setup', title: '', body: '', checks: [] }], open: 0 }; }
...
function renderBuilder() {
  ...
  ed.innerHTML = `<div><h1>Strategy builder</h1><p class="sub">Write your strategy one step at a time. It publishes as its own page on your profile and in Discover.${d.forkOf ? ` Forked from ${esc(S(d.forkOf)?.title || '')}.` : ''}</p></div>
...
function stepEditor(st, i, open) {
  ...
  return `<div class="stepcard"><header><span class="n">${String(i + 1).padStart(2, '0')}</span><button class="t" data-act="step-open" data-i="${i}" aria-expanded="${open}" style="text-align:left">${esc(st.title || 'Untitled step')}<small>${esc(st.kind)}</small></button>
    ...
    ${open ? `<div class="ed"><div class="bld-grid2"><label class="field"><span>Type</span><select class="input" data-s="kind" data-i="${i}">${KINDS.map((k) => `<option ${k === st.kind ? 'selected' : ''}>${k}</option>`).join('')}</select></label>
      ...
      <div class="field"><span>Example</span><div class="media-pick">${st.img || st.chart ? `<div class="pv">${stepShot(st, 'ed' + i).replace('class="shot"', '')}</div>` : ''}
        <label class="btn btn-line btn-sm">Upload image<input type="file" accept="image/*" class="sr" data-act="step-img" data-i="${i}"></label>
        <button class="btn btn-line btn-sm" data-act="step-chart" data-i="${i}">${st.chart ? 'New example chart' : 'Use example chart'}</button>
        ${st.img || st.chart ? `<button class="btn btn-line btn-sm" data-act="step-nomedia" data-i="${i}">Remove</button>` : ''}</div></div>
    </div>` : ''}</div>`;
}
let _pvT;
function renderPreview() { clearTimeout(_pvT); _pvT = setTimeout(() => { const pv = $('#bldPv'); if (!pv) return; const d = state.draft; pv.innerHTML = strategyHTML({ ...d, id: d.editing || slug(d.title) }, { preview: true }); }, 60); }
function publishDraft() {
  ...
  const out = { ...JSON.parse(JSON.stringify(d)), stats: { winRate: num(d.stats.winRate), avgR: num(d.stats.avgR), sample: num(d.stats.sample) } };
  d.steps.forEach((s, i) => { if (s.img) out.steps[i].img = s.img; });
  delete out.open;
  ...
  else { let id = slug(d.title); while (S(id)) id += '-2'; out.id = id; out.created = 0; if (d.forkOf) { const src = S(d.forkOf); src && src.forks++; } state.strategies.unshift(out); }
...
document.addEventListener('click', (e) => {
  ...
  switch (a) {
    ...
    case 'cmp-clear': cmp.file = null; cmp.ftype = null; $('#cmpFile').value = ''; cmpUpdate(); break;
    case 'like': { const on = !state.liked.has(id); on ? state.liked.add(id) : state.liked.delete(id); $$(`[data-act="like"][data-id="${id}"]`).forEach((b) => { b.setAttribute('aria-pressed', on); b.querySelector('span').textContent = compact(P(id).likes + (on ? 1 : 0)); }); break; }
    case 'save': { const on = !state.saved.has(id); on ? state.saved.add(id) : state.saved.delete(id); $$(`[data-act="save"][data-id="${id}"]`).forEach((b) => b.setAttribute('aria-pressed', on)); toast(on ? 'Saved' : 'Removed from saved'); break; }
    case 'focus-cmt': $(`#c-${id}`)?.focus(); break;
    case 'toggle-replay': { e.stopPropagation(); const cv = el.closest('.media').querySelector('canvas'); cv._rp.toggle(); cv._rp._upd(); break; }
    ...
    case 'follow-strat': { const on = !state.followedStrategies.has(id); on ? state.followedStrategies.add(id) : state.followedStrategies.delete(id); const s = S(id); s.followers += on ? 1 : -1; render(true); toast(on ? 'Following strategy' : 'Unfollowed strategy'); break; }
    case 'fork': location.hash = '#/builder?fork=' + id; break;
    case 'follow': { const h = el.dataset.h; state.following.has(h) ? state.following.delete(h) : state.following.add(h); render(true); break; }
    ...
    case 'step-chart': d.steps[i].img = null; d.steps[i].chart = { seed: Math.floor(Math.random() * 1e5), side: Math.random() > 0.5 ? 'long' : 'short', outcome: 'win' }; renderBuilder(); break;
    case 'step-nomedia': d.steps[i].img = null; d.steps[i].chart = null; renderBuilder(); break;
...
document.addEventListener('change', (e) => {
  ...
  if (t.dataset.s === 'kind') { state.draft.steps[+t.dataset.i].kind = t.value; renderPreview(); }
  if (t.dataset.act === 'step-img' && t.files[0]) { if (!t.files[0].type.startsWith('image/')) { $('#bldErr').textContent = 'Examples need to be an image file.'; return; } const st = state.draft.steps[+t.dataset.i]; st.img = URL.createObjectURL(t.files[0]); st.chart = null; renderBuilder(); }
});
...
function render(keepScroll = false) {
  ...
  destroyReplays(); closeModal(); Landing.destroy();
  ...
  else if (r === 'u') { if (lastRoute !== location.hash.split('?')[0]) profTab = 'posts'; html = profileView(parts[1] || ME); }
  else if (r === 'builder') html = builderView(params);
  else html = shell('floor', `<div class="page"><div class="empty">That page is not on the floor. <br><a class="btn btn-floor" href="#/floor">Go to the feed</a></div></div>`);
  app.innerHTML = html; document.title = r === '' ? 'The Trading Floor' : `${({ floor: 'The Floor', replays: 'Replays', discover: 'Discover strategies', s: S(parts[1])?.title || 'Strategy', u: '@' + (parts[1] || ME), builder: 'Strategy builder' })[r] || 'Not found'} | The Trading Floor`;
  if (r === '') Landing.init();
